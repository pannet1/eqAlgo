...
class User(object):
    ...
    def stop_and_reverse(self, symbol:str, opposite:str, percent:float=1.0):
        ...
        """
        positions = self.broker.positions()
        mis_pos   = self.broker.filter(positions, symbol=symbol, product='MIS')
        nrml_pos  = self.broker.filter(positions, symbol=symbol, product='NRML')

        if len(mis_pos) == 0:
            print(f"No MIS positions for the given {symbol}")
            is_mis = False
        else:
            positions = mis_pos[0] # since there would only be one position matching this type
            quantity = positions.get('quantity', 0)
            is_mis = False if quantity == 0 else True
        
        if not is_mis:
            if len(nrml_pos) == 0:
                print(f"No NRML positions for the given {symbol}")
                return 
            else: 
                positions = nrml_pos[0] # since there would only be one position matching this type
                quantity = positions.get('quantity', 0)
                if quantity == 0:
                    print(f"Nothing to exit for {symbol} since positions are zero")
                    return
